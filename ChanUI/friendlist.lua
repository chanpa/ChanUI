...
function CUI:ShowFriends()
    CreateFriendsTable()
    if not self.friendRoot then CreateFriendRoot() end
...
function CreateFriendsOnlineFontString()
    if not CUI.friendRoot then return end
...
function ShowFriendlist()
    ...
    -- create list
    local cols = 8
    CUI.friendList = QT:Acquire("ChanUIFriendListFrame", cols, "LEFT", "LEFT", "LEFT", "LEFT", "LEFT", "CENTER", "CENTER", "LEFT")
    ...
    for _, client in pairs(clientOrder) do
        ...
        if friends then
            ...
            CUI.friendList:SetCell(line, 1, clientTranslations[client], headlineFont, "LEFT", cols, QT.LabelProvider, -1)
...
function CheckMissingClients()
    ...
    for friendClient, friends in pairs(CUI.friendsTable) do
        ...
        if isMissing then missing[friendClient] = friends end
    end
    
    if next(missing) == nil then CUI:Print("No missing games") return end
